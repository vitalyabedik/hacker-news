...
export const CustomLink: React.FC<Props> = ({ href, target, text, variant = 'default' }) => {
  const withFocus =
    variant !== 'default' &&
    'shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg focus:ring-indigo-500 focus:ring-offset-indigo-200'
  const isBordered =
  ...
  return (
    <Link
      className={`py-2 mr-4 cursor-pointer ${isDefault} ${isColored} ${isBordered} ${withFocus} transition ease-in duration-200 text-center text-base font-semibold`}
      href={href}
      target={target}
    >
      {text}
